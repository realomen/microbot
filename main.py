            ...
            for m in event.get("markets", []):
                if is_fifty_fifty_market(m):
                    if risk.can_trade(m["id"]):
                        ...
                        count += 1
        logger.info("Scan completed", markets_found=count)
    except Exception as e:
        logger.error("Scan error", error=str(e))

# Добавляем задачи в background scheduler
...
if __name__ == "__main__":
    ...
    # Стартовое сообщение через синхронный Bot (самый надёжный способ)
    try:
        ...
        bot.send_message(
            ...
            text=f"🚀 Polymarket 50/50 MicroBot запущен успешно!\n"
                 ...
                 f"Ставка: ${settings.BET_SIZE_USD}"
        )
        logger.info("Startup message sent to Telegram")
    except Exception as e:
        logger.warning("Startup message failed (check chat_id)", error=str(e))

    # Telegram polling в ГЛАВНОМ потоке — это решает set_wakeup_fd
